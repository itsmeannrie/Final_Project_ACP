...
def sign_up():
    ...
    USER_CREDENTIALS[username] = {"name": name, "password": hashed_password}
...
def log_in():
    ...
    if username in USER_CREDENTIALS:
        if hash_password(password) == USER_CREDENTIALS[username]["password"]:
            print(f"\tWelcome, {USER_CREDENTIALS[username]['name']}!")
            return username  
        else:
